...
## Step 2: Define the linear predictors {.smaller auto-animate=true}
...
predErie.bru <- predict(fit_greatLakes,
                      pred.df,
                       height  ~  exp(beta0_erie + GP_erie + GP_shared ) ,
                      n.samples = 1000
)
...
predMich.bru <- predict(fit_greatLakes,
                      pred.df,
                       height  ~   exp(beta0_mich + GP_mich + GP_shared_copy1) ,
                      n.samples = 1000
)

predClair.bru <- predict(fit_greatLakes,
                      pred.df,
                       height  ~   exp(beta0_clair + GP_clair + GP_shared_copy2),
                      n.samples = 1000
)
...
- **Marked point Processes**: [@laxton2023balancing] Model habitat availability (wetland locations) using a point process model and occupancy probability as marks.
